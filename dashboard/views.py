...
# from rest_framework.response import Response
# from datetime import date, timedelta
# from attendance.models import Darajah, Hizb, HizbGroup, AttendanceRecord, Student, AttendanceFlag
...
# def get_role_filtered_records(user, start, end):
#     qs = AttendanceRecord.objects.filter(date__range=(start, end)).select_related(
#         "student", "student__darajah", "student__hizb"
#     )

#     if user.role == "admin":
#         return qs

#     elif user.role in ["prefect", "deputy_prefect"] and user.hizb_id:
#         return qs.filter(student__hizb_id=user.hizb_id)

#     elif user.role == "masool" and user.darajah_id:
#         return qs.filter(student__darajah_id=user.darajah_id)

#     elif user.role == "muaddib":
#         return qs.filter(student__in=user.students.all())

#     elif user.role == "lajnat_member":
#         flagged_ids = AttendanceFlag.objects.filter(assigned_to=user).values_list(
#             "attendance_record_id", flat=True
#         )
#         return qs.filter(id__in=flagged_ids)
...
#     # ✅ Filter records by role before aggregation
#     records = get_role_filtered_records(user, start, end)

#     # Build daily/weekly/rooms/flagged data using filtered records
#     # If your aggregator functions accept a queryset, pass `records`
#     # Otherwise, adapt them to accept `role` + queryset
#     daily_data = daily(start, end, role, records=records)
#     weekly_data = weekly(start, end, role, records=records)
#     rooms_data = room_ranking(start, end, role, records=records)
#     flagged_data = flagged_rooms(start, end, role, threshold, records=records)
